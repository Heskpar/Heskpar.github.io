...
const InterfaceEdgePrototype = {
	...
	if((dx>=dy && dx>= -dy)||(dx<dy && dx<-dy)) {
	    ...
	    ctx.lineTo(startPoint.x-dx+(9*Math.sign(dx)),startPoint.y-dy)
	    ctx.stroke()
	    ctx.beginPath()
	    ctx.setLineDash([])
	...
	//if the other node is to the left or right
	    ...
	    ctx.lineTo(endPoint.x,endPoint.y+(9*Math.sign(dy)))
...
function createInterfaceEdge(startNode,endNode)
{
    var result = Object.create(InterfaceEdgePrototype)
